    ...
    return class OracleBuildConfig {
        ...
        preBuild(){
            ...
            if (!isSavedFile){
                ...
                //the script being compiled is the 4th element in the array
                const SQL_SCRIPT_INDEX = 3;

                let untitledFile = temp.openSync({suffix: '.sql'});
                // console.log(untitledFile);
                _untitledFileDest = untitledFile.path;

                fs.writeFileSync(untitledFile.fd, fileBuffer.buffer.cachedText);

                this.args[SQL_SCRIPT_INDEX] = untitledFile.path;
                delete this.cwd;
            }
        }
